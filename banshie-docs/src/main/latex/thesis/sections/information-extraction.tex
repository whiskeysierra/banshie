...
\fxfatal{How are these definitions relevant?}
...
\fxfatal{Add location to example?}
...
The method of Knowledge Engineering is the manual creation of grammar by a human expert. The identification can be done either by comparing them to a list or by applying rules. \fxfatal{Reformulate} Domain knowledge, which is not always available, is necessary to specify extraction rules in which case the method of knowledge engineering can not be applied. Experts must find patterns by inspecting the corpus and produce guidelines according to these patterns \cite{Schramm:2008}\cite{Turmo:2006}.
...
\fxfatal{Information without semantics}
\fxfatal{New information may require ontology to be modified}
\fxfatal{Mixed approach}
...
\fxfatal{Weak, requires more or better information}
