...
\begin{figure}[H]
\centering
\includegraphics[angle=90, width=0.31\textwidth]{module-dependencies.png}
\caption{Module dependencies}
\label{fig:module-dependencies}
\end{figure}

The most important module, as shown in figure \ref{fig:module-dependencies} is the \textit{banshie-api} module. It contains the public \gls{API} of the framework which itself is described in chapter \ref{sec:api} (see pp. \pageref{sec:api}ff.). The \gls{API} module is the only module clients or users of the framework should directly depend on. All the other modules form the implementation of the framework's public \gls{API} and are highly implementation specific.

The next most important modules are \textit{banshie-corpora}, \textit{banshie-extractors}, \textit{banshie-execution}, \textit{banshie-eval-performance} and \textit{banshie-eval-quality}. They are based on the package hierarchy of the \gls{API} and provide the default implementation of their corresponding packages.

Since a single implementation module for some of these packages would violate the \textit{Single Responsibility Pattern} (s. chapter \ref{sec:patterns}) they had to be broken up into smaller ones. The results of this process are the following modules: \textit{banshie-process-api}, \textit{banshie-process-impl}, \textit{banshie-monitor-api}, \textit{banshie-monitor-impl},  \textit{banshie-event-api}, \textit{banshie-event-impl}, \textit{banshie-logging-api} and \textit{banshie-logging-impl}. The naming schema indicates that all of these modules have been split up into their own respective \gls{API} and implementation modules, which can be exchanged by alternative versions if desired. The \gls{API} modules form an internal \gls{API} which allows implementation modules to communicate through a well-defined interface, those modules must not be confused with the public \gls{API} module \textit{banshie-api}. The different responsibilities and respective \gls{API}s will be discussed in chapter \ref{sec:api}. It should also be noted, that figure \ref{fig:module-dependencies} nicely demonstrates the result of applying the patterns discussed in chapter \ref{sec:patterns}: Every module only depends on \gls{API} (or library) modules, but never on implementation modules.

Another group of modules contains the \textit{banshie-common}, \textit{banshie-jpa} and \textit{banshie-persistence} modules. The \acl{JPA} module provides persistence-related utility features while the common module just offers more general framework-related features. The persistence module is not so much the result of a pattern-based design approach but rather the consequence of \gls{OSGi}'s bad support for \gls{JPA}. Aggregating domain model and persistence-related classes into one module makes integrating them into an \gls{OSGi} container much easier, since all relevant classes are loaded from a single classloader.
...
\citeauthor{Douthat:1998} proposed the original version of the \enquote{General Greedy Mapping Algorithm} in 1998 \cite{Douthat:1998}. It's based on finding matching pairs of spans in the reference and the predication. Evaliex even extended this algorithm by basing the matching on string or word similarity algorithms like Levenshtein-distance or the Jaccard-coeffecient \cite{Linsmayr:2010}. But since Banshie, in its current version, focuses solely on the \textit{All Occurences} approach of \gls{IE} (see \ref{sec:ie-approaches}, pp. \pageref{sec:ie-approaches}ff.), a simpler algorithm to associate reference and hypothesis has been used. Pairs are matched based on character offsets calculated from the original document. This way an extractor is required to find exact or partial matches (\textit{overlap} and \textit{contains} rule) of spans defined in the reference to score in the evaluation metrics. See \ref{sec:matching-rules}, p. \pageref{sec:matching-rules}, for a detailed explanaition of the different matching rules.
...
\subsection{Implementation obstacles}
The next chapter will describe and discuss the obstacles that occured during the implementation and testing phase of the framework and how they problems that arised got solved.
