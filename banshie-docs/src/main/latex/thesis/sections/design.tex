...
Corpus, Extractor, CorpusRepository, ExtractorRepository

\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{api-model.png}
\caption{Banshie Model API}
\end{figure}
...
QualityEvaluator, PerformanceEvaluator, Dimension, Value
...
Too big, required to be split up into multiple smaller, more maintainable modules.
Process management, Process Monitor, Event Producer, JMX, Event Logger, JSON logfile

\subsubsection{Quality Evaluator}
Map prediction to reference, count true positives, false positives, false negatives, run configured scores, collect results

\subsubsection{Performance Evaluator}
Read log file line by line, call update on configured calculators, collect results
