...
Banshie's domain model has been designed with simplicity and extensibility in mind. The two only entity classes, \texttt{Corpus} and \texttt{Extractor}, are merely containers for file locations and very little meta data, but since the persistence layer is based on \gls{JPA}, which is based on POJOs, adding properties is a rather easy task.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{api-model.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.65\textwidth]{logging.png}
