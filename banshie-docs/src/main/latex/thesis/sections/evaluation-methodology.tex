\section{Formal \gls{IE} evaluation methodology}
...
\fxfatal{Formal definition, description, ...}
Depending on the intended use case of the IE system, different approaches to information extraction may be useful. A first possibility is the \enquote{OBD} approach. In this case, we see the \textit{IE} task as filling a given template structure. This is for example useful if the goal of the extractor is to fill a database in order to be able to query the information contained in the documents. \cite{Sitter:2004}.
...
\fxfatal{Rules}
% exact, contain, overlap
% quote D. Freitag (Freitag:1998)
